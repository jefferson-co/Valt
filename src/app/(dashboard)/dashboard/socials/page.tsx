...
export default function SocialsPage() {
  ...
  useEffect(() => {
    async function loadSocials() {
      ...
      const socialMap: Record<string, { id?: string; url: string }> = {};
      (data as Social[] || []).forEach((s) => {
        socialMap[s.platform] = { id: s.id, url: s.url };
      });
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    for (const platform of PLATFORMS) {
      const current = socials[platform.key];
      const url = current?.url?.trim() || "";

      if (current?.id && !url) {
        // Delete existing social
        await supabase.from("socials").delete().eq("id", current.id);
      } else if (current?.id && url) {
  ...
  if (loading) {
    return (
      <div>
        <h1 className="font-display text-2xl font-bold">Socials</h1>
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-muted-foreground">
        Add your social media links. These will appear as icons on your public
        page.
      </p>
